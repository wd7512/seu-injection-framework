...
1. Find the "Build and Deploy Sphinx Documentation" workflow
1. Wait for it to complete (should take 2-3 minutes)
1. Visit https://wd7512.github.io/seu-injection-framework/
1. You should see the documentation homepage
...
1. **Deploy Job** (only on `main` branch):
...
1. Check that the workflow has permission to deploy (should be automatic with `pages: write` permission)
...
1. The changes are merged to the `main` branch
...
1. Update the Makefile to copy CNAME to build output
1. Configure DNS records to point to GitHub Pages
1. In repository Settings > Pages, set the custom domain
