...
def prepare_dataset(dataset_name="moons", n_samples=2000, noise=0.3, random_state=42):
    ...
    if dataset_name == "moons":
        ...
        X, y = make_blobs(
            n_samples=n_samples, centers=2, n_features=2, cluster_std=noise * 5, random_state=random_state
        )
    else:
    ...
    for dataset_name in datasets:
        ...
        for use_dropout in dropout_configs:
            ...
            for flood_level in flood_levels:
                ...
                _, seu_results = evaluate_seu_robustness(model, X_test, y_test, sampling_rate=sampling_rate)
...
def analyze_results(results):
    ...
    for dataset in ["moons", "circles", "blobs"]:
        ...
        for config, label in [(with_dropout, "With Dropout"), (without_dropout, "Without Dropout")]:
            ...
            print(
                f"    Standard: Acc={standard['baseline_accuracy']:.3f}, SEU Drop={standard['mean_accuracy_drop']:.3f}"
            )
            ...
            for r in sorted(config, key=lambda x: x["flood_level"]):
                ...
                improvement = (
                    -drop_change / standard["mean_accuracy_drop"] * 100 if standard["mean_accuracy_drop"] > 0 else 0
                )
