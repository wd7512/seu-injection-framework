...
def train_fault_aware_model(
    model, X_train, y_train, epochs=100, lr=0.01, fault_prob=0.005, fault_freq=10, verbose=True
):
    """
    ...
    for epoch in pbar:
        ...
        if verbose and (epoch + 1) % 25 == 0:
            pbar.set_postfix({"loss": f"{loss.item():.4f}", "faults": len(fault_epochs)})
...
def evaluate_robustness(model, X_test, y_test, model_name="Model", bit_positions=[0, 1, 8, 15, 23], sample_rate=0.1):
    """
    ...
    results = {"model_name": model_name, "baseline_accuracy": baseline_acc, "bit_results": {}}
...
def plot_training_comparison(baseline_losses, fault_losses, fault_injections):
    ...
    ax.plot(epochs, baseline_losses, label="Baseline (No Faults)", linewidth=2, alpha=0.8)
    ax.plot(epochs, fault_losses, label="Fault-Aware Training", linewidth=2, alpha=0.8)

    # Mark fault injection events
    for inj_epoch in fault_injections:
        ax.axvline(x=inj_epoch, color="red", linestyle="--", alpha=0.3, linewidth=1)

    ax.set_xlabel("Epoch", fontsize=12)
    ax.set_ylabel("Training Loss", fontsize=12)
    ax.set_title("Training Loss Comparison: Baseline vs Fault-Aware", fontsize=14, fontweight="bold")
...
def plot_robustness_comparison(baseline_results, fault_results):
    ...
    baseline_drops = [baseline_results["bit_results"][b]["accuracy_drop"] * 100 for b in bit_positions]
    fault_drops = [fault_results["bit_results"][b]["accuracy_drop"] * 100 for b in bit_positions]
    ...
    bars1 = ax.bar(x - width / 2, baseline_drops, width, label="Baseline Model", color="coral", alpha=0.8)
    bars2 = ax.bar(x + width / 2, fault_drops, width, label="Fault-Aware Model", color="skyblue", alpha=0.8)

    ax.set_xlabel("Bit Position (IEEE 754)", fontsize=12)
    ax.set_ylabel("Accuracy Drop (%)", fontsize=12)
    ax.set_title("Robustness Comparison: Accuracy Drop Under Bit Flips", fontsize=14, fontweight="bold")
    ax.set_xticks(x)
...
def run_complete_experiment():
    ...
    baseline_model, baseline_losses = train_baseline_model(baseline_model, X_train, y_train, epochs=100)
    ...
    baseline_results = evaluate_robustness(
        baseline_model, X_test, y_test, model_name="Baseline Model", bit_positions=bit_positions, sample_rate=0.1
    )

    fault_results = evaluate_robustness(
        fault_model, X_test, y_test, model_name="Fault-Aware Model", bit_positions=bit_positions, sample_rate=0.1
    )
